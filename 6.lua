...
local test_input = iter.map(tonumber)(iter.values({ "3", "4", "3", "1", "2" }))
local input = iter.map(tonumber)(string.gmatch(io.read("l"), "(%d+),?"))
...
for number in input do
    generations[0][number] = (
            generations[0][number] and generations[0][number] or 1
        ) + 1
end

for i = 1, 256 do
    local generation = {}
    local parent = generations[i - 1]

    for i = 1, 8 do
        generation[i - 1] = parent[i] or 0
    end

    generation[6] = generation[6] + parent[0]
    generation[8] = parent[0]
...
for i = 0, 8 do
    total = total + generations[#generations][i]
end

print(
    string.format("Total number of fish after %2d days:", #generations),
    total
)
